...
## Detection Ideas
- Alert on off-hours interactive logons from new geolocations tied to privileged accounts.
- Correlate PowerShell/script execution with outbound connections on the same host.
- Watch for staging file creation under ProgramData or Temp with repeated hashes.
- Detect scheduled task and Run key persistence tied to maintenance-themed names.
- Flag startup folder executable drops followed by execution.
- Monitor Defender exclusion attempts in administrative or staging paths.
...
- **CorpHealth Operations Activity Review (MDE Advanced Hunting CTF)** — Incident investigation write-up with timeline, IOCs, detections, and KQL workflows. [View repo](https://github.com/CARLOSFUN/corphealth-traceback-threat-hunt)

Copy-ready snippet for your profile README:
```markdown
- **CorpHealth Operations Activity Review (MDE Advanced Hunting CTF)** — Incident investigation write-up with timeline, IOCs, detections, and KQL workflows. [View repo](https://github.com/CARLOSFUN/corphealth-traceback-threat-hunt)
```
