...
## Detection Ideas
- Off-hours interactive logon with new geography (`DeviceLogonEvents`).
- Script execution that initiates outbound connections (`DeviceProcessEvents`, `DeviceNetworkEvents`).
- Staging file creation in ProgramData/Temp paths (`DeviceFileEvents`).
- Persistence via scheduled task cache or run keys (`DeviceRegistryEvents`).
- External binary downloads via ngrok domains (`DeviceNetworkEvents`).
